...
struct segment { LL num , bit; } Tree[ 4*N ];
...
LL Query( int node , int b , int e , int i , int j ) {
    ...
    if( i <= b && j >= e ) return Tree[node].num;
...
void Build( int node , int b , int e ) {
    ...
    Build( left , b , mid );
    Build( right , mid+1 , e );
    if( Tree[left].bit == Tree[right].bit ) {
        Tree[node].num = min( Tree[left].num , Tree[right].num );
        Tree[node].bit = Tree[left].bit;
    }
    else if( Tree[left].bit > Tree[right].bit ) {
        Tree[node].num = Tree[left].num;
        Tree[node].bit = Tree[left].bit;
    }
    else {
        Tree[node].num = Tree[right].num;
        Tree[node].bit = Tree[right].bit;
    }
}
int main( int argc , char const *argv[] ) {
    ...
    FOR( i , 1 , n ) X[i] = ( ara[i]^x );
    FOR( i , 1 , n ) val[i] = ONE( X[i] );
    while( q-- ) {
        int l , r;
        sc("%d %d",&l,&r);
        LL B = 0 , id = INF;
