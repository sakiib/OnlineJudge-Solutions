...
int main( int argc , char const *argv[] ) {
    ...
    for( int tc = 1; tc <= t; tc++ ) {
        LL n , a , b , c;
        sc("%lld %lld %lld %lld",&n,&a,&b,&c);
        if( n <= 2 ) {
            pf("Case %d: 0\n",tc); continue;
        }
        original.M[1][1] = a , original.M[1][3] = b;
        original.M[1][4] = 1 , original.M[2][1] = 1;
        original.M[3][2] = 1 , original.M[4][4] = 1;
        Matrix res = MatrixExpo( original , n-2 );
        LL ans = (res.M[1][4]%mod*c%mod)%mod;
        pf("Case %d: %lld\n",tc,ans);
    }
    return 0;
}
