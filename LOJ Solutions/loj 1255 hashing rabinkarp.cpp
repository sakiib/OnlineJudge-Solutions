...
char pattern[ 10*N ] , text[ 10*N ];
uLL patlen , texlen;
uLL hashing[ 10*N ];
uLL pw , base , texhash;

void RollingHashRK( ) {
    patlen = strlen( pattern ) , texlen = strlen( text );
    base = 31 , pw = 1;
    hashing[0] = 0;
    texhash = 0;
    for( int i = 0; i < patlen; i++ ) {
        hashing[i+1] = hashing[i]*base+(pattern[i]-'a'+1);
    }
    for( int i = 0; i < texlen; i++ ) {
        texhash = texhash*base+(text[i]-'a'+1);
        pw *= base;
    }
}
int main( int argc , char const *argv[] ) {
    ...
    for( int tc = 1; tc <= t; tc++ ) {
        ...
        RollingHashRK( );
        int ans = 0;
        for( int i = 0; i < patlen-texlen+1; i++ ) {
            uLL val = hashing[i+texlen]-(pw*hashing[i]);
            if( val == texhash ) ans++;
        }
        pf("Case %d: %d\n",tc,ans);
    }
