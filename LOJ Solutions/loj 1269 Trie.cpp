...
int GetMaxXor( int x ) {
    ...
    for( int i = 31; i >= 0; i-- ) {
        int id = Check( x , i );
        if( cur -> Child[1-id] != NULL ) cur = cur -> Child[1-id];
        else if( cur -> Child[id] != NULL ) cur = cur -> Child[id];
    }
    return ( x^cur->val );
}
int GetMinXor( int x ) {
    ...
    for( int i = 31; i >= 0; i-- ) {
        int id = Check( x , i );
        if( cur -> Child[id] != NULL ) cur = cur -> Child[id];
        else if( cur -> Child[1-id] != NULL ) cur = cur -> Child[1-id];
    }
    return ( x^cur->val );
}
...
int main( int argc , char const *argv[] ) {
    ...
    for( int tc = 1; tc <= t; tc++ ) {
        ...
        res = 0;
        Insert( 0 );
        int cumx = 0 , mx = 0 , mn = inf;
        for( int i = 1; i <= n; i++ ) {
            sc("%d",&ara[i]);
            cumx ^= ara[i];
            mx = max( mx , GetMaxXor( cumx ) );
            mn = min( mn , GetMinXor( cumx ) );
            Insert( cumx );
        }
        pf("Case %d: %d %d\n",tc,mx,mn);
        Delete( Root );
