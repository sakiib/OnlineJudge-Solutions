...
vector <ll> V;
vector <LL> prime;
vector <bool> isprime( 400*N , true );

void sieve( ) {
    isprime[0] = isprime[1] = false;
    prime.push_back( 2 );
    for( LL i = 4; i <= 400*N; i +=2 ) isprime[i] = false;
    LL sq = sqrt( 400*N );
    for( LL i = 3; i <= sq; i += 2 ) {
        if( isprime[i] ) {
            for( LL j = i*i; j <= 400*N; j += 2*i ) {
                isprime[j] = false;
            }
        }
    }
    for( LL i = 3; i <= 400*N; i += 2 ) {
...
int main( int argc , char const *argv[] ) {
    ...
    while( sc("%lld",&n) ) {
        ...
        FOR( i , 0 , sz(V)-1 ) {
            if( i == 0 ) pf("%lld^%lld",V[i].F,V[i].S);
            else pf(" %lld^%lld",V[i].F,V[i].S);
        }
        pf("\n") , clr(V) , clr(M);
    }
    return 0;
