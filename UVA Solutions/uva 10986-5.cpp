...
void dijkstra( int s ) {
    ...
    while( !Q.empty( ) ) {
        ...
        FOR( i , 0 , sz(graph[s])-1 ) {
            ii cur = graph[s][i];
            int cc = cur.S , ss = cur.F;
            if( cost[ss] == inf || cost[ss] > cost[s]+cc ) {
                cost[ss] = cost[s]+cc;
                Q.push( mp(cost[ss],ss) );
            }
        }
    }
}
int main( int argc , char const *argv[] ) {
    ...
    FOR( tc , 1 , t ) {
        ...
        dijkstra( st );
        if( cost[en] == inf ) pf("Case #%d: unreachable\n",tc);
        else pf("Case #%d: %d\n",tc,cost[en]);
        CLR( graph , N );
    }
    return 0;
