...
int solve( int mask , int taken ) {
    cout << mask << " " << taken << endl;
    if( taken > k ) return -inf;
    if( mask == ( 1 << k )-1 ) return 0;
    if( taken == k ) return 0;
    if( dp[mask][taken] != -1 ) return dp[mask][taken];
    int ret = 0;
    for( int i = 0; i < n; i++ ) {
        if( !Check( mask , taken ) && taken < k )  {
            ret = max( ret , solve( Set( mask,taken ) , taken+1 )^a[i] );
        }
    }
    return dp[mask][taken] = ret;
}
