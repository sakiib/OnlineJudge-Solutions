...
uLL needle_size , haystack_size , base , pw , needle_hash;

uLL hashing[ 10*N ];

char needle[ 10*N ] , haystack[ 10*N ];

void RollingHash_RK( ) {
    needle_hash = 0 , base = 31 , pw = 1;
    hashing[0] = 0;
    for( int i = 0; i < haystack_size; i++ ) {
        hashing[i+1] = hashing[i]*base+(haystack[i]-'a'+1);
    }
    for( int i = 0; i < needle_size; i++ ) {
        needle_hash = needle_hash*base+(needle[i]-'a'+1);
        pw *= base;
    }
}
int main( int argc , char const *argv[] ) {
    ...
    while( sc("%llu",&needle_size) != EOF ) {
        sc("%s %s",needle,haystack);
        ...
        uLL val = 0;
        //cout << "main val : " << needle_hash << endl;
        for( int i = 0; i < haystack_size-needle_size+1; i++ ) {
            val = hashing[i+needle_size]-(pw*hashing[i]);
            //cout<<"val : "<<val<<" pos from : "<<i<<" "<<i+needle_size<<endl;
            if( val == needle_hash ) pf("%d\n",i) , F = false;
        }
        if( F ) pf("\n");
    }
    return 0;
