...
int cnt[ 100*N ] , MN[ 100*N ] , has[ 100*N ];
vector <int> temp;
...
int main( int argc , char const *argv[] ) {
    int n;
    for( LL i = 2; i < 100*N; i++ ) {
        if( !lp[i] ) {
            for( LL j = i; j < 100*N; j += i ) {
                lp[j] = i;
            }
        }
    }
    for( int i = 1; i < 100*N; i++ ) MN[i] = inf;
    ...
    for( int i = 1; i <= n; i++ ) {
        ...
        for( int i = 0; i < temp.size(); i++ ) {
            has[ temp[i] ]++;
            MN[ temp[i] ] = min( MN[ temp[i] ] , cnt[ temp[i] ] );
            cnt[ temp[i] ] = 0;
        }
    }
    LL ans = 1;
    for( LL i = 2; i < 100*N; i++ ) {
        if( has[i] == n ) {
            ans = ( ans%MOD * Bigmod( i , MN[i]*1LL , MOD*1LL )%MOD ) % MOD;
        }
    }
    printf("%lld\n",ans);
    return 0;
