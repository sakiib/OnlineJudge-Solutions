...
/**
 here indegree is actually outdegree of the original graph ,
 & indegree of the reversed graph.
 */

void TopSort( int s ) {
    vis[s] = true;
    FOR( i , 0 , sz( graph[s] )-1 ) {
        int k = graph[s][i];
        if( !vis[k] ) TopSort( k );
    }
    S.push( s );
...
int main( int argc , char const *argv[] ) {
    ...
    FOR( i , 1 , n ) if( !vis[i] ) TopSort( i );
    MEM( vis , false );
    scc = 0;
    while( !S.empty( ) ) {
        int tp = S.top();
        S.pop();
        if( !vis[tp] ) scc++ , Kosaraju( tp );
    }
    FOR( i , 1 , n ) {
    ...
    int zerodegree = 0;
    FOR( i , 1 , scc ) {
        if( indegree[i] == 0 ) zerodegree++;
    }
    if( zerodegree > 1 ) {
        return pf("0\n") , 0;
    }
    int cap = 0;
    FOR( i , 1 , n ) if( indegree[ SCC[i] ] == 0 ) cap++;
    pf("%d\n",cap);
    FOR( i , 1 , n ) {
        if( indegree[ SCC[i] ] == 0 ) pf("%d ",i);
    }
    pf("\n");
    return 0;
}
