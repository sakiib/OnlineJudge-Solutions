...
int Query( Node* node , int b , int e , int i , int j ) {
        ...
        int mid = ( b + e ) >> 1;
        int q1 = Query( node -> l , b , mid , i , j ) , q2 = Query( node -> r , mid+1 , e , i , j );
        return ( q1 + q2 );
}
void update( Node* prev , Node* cur , int b , int e , int pos , int nval ) {
        if( b > e || pos > e || pos < 0 ) return;
        if( b == e && b == pos ) { cur -> val = ( prev -> val + nval ); return; }
        int mid = ( b + e ) >> 1;
        if( pos <= mid ) {
                cur -> r = prev -> r , cur -> l = new Node( NULL , NULL , 0 );
                update( prev -> l , cur -> l , b , mid , pos , nval );
        }
        else {
                cur -> l = prev -> l , cur -> r = new Node( NULL , NULL , 0 );
