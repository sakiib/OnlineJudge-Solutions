...
int main( int argc , char const *argv[] ) {
    ...
    FOR( i , 1 , 26 ) {
        FOR( j , 1 , 26 ) {
            if( i == j ) cost[i][j] = 0;
            else cost[i][j] = inf , taken[i][j] = inf;
        }
    }
    FOR( i , 1 , n ) {
        string x , y ;
        int c;
        cin >> x >> y >> c;
        char a = x[0]-'a'+1 , b = y[0]-'a'+1;
        cost[a][b] =  min( cost[a][b] , c );
    }
    if( sz(a) != sz(b) ) return pf("-1\n") , 0;
    FOR( k , 1 , 26 ) {
        FOR( i , 1 , 26 ) {
            FOR( j , 1 , 26 ) {
    ...
    FOR( i , 0 , sz(a)-1 ) {
        ...
        if( x == y ) { res += a[i] ; continue; }
        int mn = inf , id = 0;
        FOR( j , 1 , 26 ) if( cost[x][j]+cost[y][j] < mn ) mn = cost[x][j]+cost[y][j] , id = j;
        res += ( id+'a'-1 );
        if( mn == inf ) return pf("-1\n") , 0;
        ans += mn;
    }
    pf("%d\n",ans);
    cout << res << endl;
