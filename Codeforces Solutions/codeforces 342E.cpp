...
int ans[ centroid.MAXN ];

void Update( int u ) {
        ans[ u ] = 0;
        int p = centroid.CentroidParent[ u ];
        while( p != -1 ) {
                ans[ p ] = min( ans[ p ] , ans[ u ] + centroid.M[ p ][ u ] );
                p = centroid.CentroidParent[ p ];
        }
}
int Query( int u ) {
        int ret = ans[ u ];
        int p = centroid.CentroidParent[ u ];
        while( p != -1 ) {
                ret = min( ret , ans[ p ] + centroid.M[ p ][ u ] );
                p = centroid.CentroidParent[ p ];
        }
        return ret;
}
int main( int argc , char const *argv[] ) {
        int n , q;
        scanf("%d %d",&n,&q);
        centroid.Initialize( n );
        for( int i = 1; i<= n-1; i++ ) {
                int u , v;
                scanf("%d %d",&u,&v);
                centroid.AddEdge( u , v );
        }
        for( int i = 1; i <= n; i++ ) ans[i] = centroid.INF;
