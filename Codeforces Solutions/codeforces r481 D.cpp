...
void check( ) {
    ...
    LL last = y;
    for( int i = 3; i <= n; i++ ) {
        if( ara[i]-last == dif ) last = ara[i];
        else if( ara[i]+1-last == dif ) last = ara[i]+1 , ans++;
        else if( ara[i]-1-last == dif ) last = ara[i]-1 , ans++;
        else {
            f = false;
            //cout << "here : " << endl;
...
int main( int argc, char const *argv[] ) {
    ...
    x = ara[1]+1 , y = ara[2] , ans = 1;
    check( );
    ans = 0;

    x = ara[1]+1 , y = ara[2]+1 , ans = 2;
    check( );
    ans = 0;
