...
void build(LL node,LL b,LL e,LL op) {
    if(b>e) return;
    if(b==e) {
        tree[node]=ara[b];
        return;
    ...
    build(left,b,mid,op-1);
    build(right,mid+1,e,op-1);
    if(op&1) tree[node]=tree[left] | tree[right];
    else tree[node]=tree[left] ^ tree[right];
}
void update(LL node,LL b,LL e,LL pos,LL val,LL op) {
    ...
    LL left=2*node; LL right=2*node+1; LL mid=(b+e)/2;
    update(left,b,mid,pos,val,op-1);
    update(right,mid+1,e,pos,val,op-1);
    if(op&1) tree[node]=tree[left] | tree[right];
    else tree[node]=tree[left] ^ tree[right];
}
