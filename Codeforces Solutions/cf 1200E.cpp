...
inline LL make_big( LL x , LL y ) { return ( x << 31 ) | y; }
...
int main( ) {
        ...
        for( int i = 2; i <= n; i++ ) {
                ...
                for( int i = la - 1 , j = 0; j < lb && i >= 0; j++ , i-- ) {
                        ...
                        X = add( ans[i] , mul( base[0] , X , MOD[0] ) , MOD[0] );
                        Y = add( ans[i] , mul( base[1] , Y , MOD[1] ) , MOD[1] );
                        LL cur_hash1 = make_big( x , y ) , cur_hash2 = make_big( X , Y );
                        if( cur_hash1 ==  cur_hash2 ) start = j + 1;
                }
                for( int j = start; j < lb; j++ ) ans += cur[j];
        }
        cout << ans << endl;
        return 0;
}
