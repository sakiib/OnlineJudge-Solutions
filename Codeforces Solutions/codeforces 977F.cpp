...
int main( int argc, char const *argv[] ) {
    ...
    FOR( i , 1 , n ) {
        ...
        if( M[ need ].size() == 0 ) {
           ans[ ara[i] ] = 1;
           if( mx < ans[ ara[i] ] ) mx = ans[ ara[i] ] , id = i;
           parent[i] = pos[ need ];
           pos[ara[i]] = i;
           continue;
        }
        int k = lower_bound(M[need].begin(),M[need].end(),need)-M[need].begin();
        ans[ ara[i] ] = ans[ M[need][k] ]+1;
        if( mx < ans[ ara[i] ] ) mx = ans[ ara[i] ] , id = i;
        parent[i] = pos[ need ];
        pos[ara[i]] = i;
        cout <<"mx :  "<<i << " "<<mx << endl;
    ...
    nl;
    //cout << id <<" "<<parent[id] <<" "<<parent[ parent[id] ]<< endl;
    return 0;
}
