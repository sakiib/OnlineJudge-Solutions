...
int solve( int id , int per ) {
    if( id > n ) return 0;
    if( dp[id][per] != -1 ) return dp[id][per];
    int res = 0;
    if( per ) {
        res = 0;
        res = max( res , ara[id]+solve( id+1 , !per ));
        res = max( res , solve( id+1 , per ));
    }
    else {
        res = inf;
        res = min( res , ara[id]+solve( id+1 , per ));
        res = min( res , solve( id+1 , !per ));
    }
    return dp[id][per] = res;
}
int main( int argc , char const *argv[] ) {
    sc("%d",&n);
    FOR( i , 1 , n ) sc("%d",&ara[i]) , tot += ara[i];
    MEM( dp , -1 );
    int bob = solve( 1 , 0 );
    pf("%d %d\n",bob,tot-bob);
    return 0;
