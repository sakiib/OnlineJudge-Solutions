...
bool dfs( int s , int p , int c ) {
    if( col[s] != c ) return false;
    bool ret = true;
    for( auto x : graph[s] ) {
        if( x == p ) continue;
        ret = ret&( dfs( x , s , c ) );
...
int main( int argc, char const *argv[] ) {
    ...
    int u , v;
    FOR( i , 1 , n-1 )
    cin >> u >> v , graph[u].pb(v) , graph[v].pb(u) , edge.pb( mp(u,v) );
    FOR( i , 1 , n ) cin >> col[i];
    int candyA , candyB;
    bool f = false;
    for( auto x : edge ) {
        if( col[x.ff] != col[x.ss] ) {
            candyA = x.ff , candyB = x.ss;
            f = true;
            break;
        }
    }
    ...
    for( auto x : graph[candyA] ) {
        ok = ok&dfs( x , candyA , col[x] );
    }
    if( ok ) return cout << "YES\n" << " " << candyA << endl , 0;

    ok = true;
    for( auto x : graph[candyB] ) {
        ok = ok&dfs( x , candyB , col[x] );
    }
    if( ok ) return cout <<"YES\n"<< candyB << endl,0;
    cout << "NO" << endl;
    return 0;
