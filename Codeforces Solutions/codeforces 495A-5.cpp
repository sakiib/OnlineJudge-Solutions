...
int main( int argc , char const *argv[] ) {
    ...
    /**
     ...
     (a-b)%y = 0;
     ....... etc
     hence , x , y , z etc are the divisor of (a-b)
     find them in sqrt(N) , then check them
     if ( a%divi(a-b) ) = b or not
    */
    LL ans = 0;
    LL dif = a-b;
    LL sq = sqrt( dif );
    FOR( i , 1 , sq ) {
        if( dif%i == 0 ) {
           V.pb(i);
           if( dif/i != i ) V.pb(dif/i);
        }
    }
    FOR( i , 0 , sz(V)-1 ) if( a%V[i] == b ) ans++;
    pf("%lld\n",ans);
    return 0;
}
