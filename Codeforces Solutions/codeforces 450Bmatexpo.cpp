...
int main( int argc , char const *argv[] ) {
    ...
    if( n == 1 ) return pf("%lld\n",(x+MOD)%MOD) , 0 ;
    if( n == 2 ) return pf("%lld\n",(y+MOD)%MOD) , 0 ;
    original.M[1][1] = 1 , original.M[1][2] = -1;
    original.M[2][1] = 1 , original.M[2][2] = 0;
    Matrix ans = MatrixExpo( original , n-2 );
    LL res = (ans.M[1][1]%MOD*y%MOD+ans.M[1][2]%MOD*x%MOD)%MOD;
    pf("%lld\n",(res+MOD)%MOD);
    return 0;
}
