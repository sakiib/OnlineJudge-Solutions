...
bool ok( string s , int x , string f ) {
    //string f = s.substr( 0 , x );
    //cout <<"now : "<<x <<" "<< f << endl;
    int l = 0 , r = x-1;
    while( r < s.size() ) {
        string k = s.substr( l , x );
        if( k != f ) return false;
        l += x , r += x;
    }
    return true;
...
int main( int argc, char const *argv[] ) {
    ...
    vector <int> d = getdiv( a.size() );
    int cnt = 0;
    for( int i = 0; i < d.size(); i++ ) {
        int div = d[i];
        if( b.size()%div != 0 ) continue;
        string temp = a.substr( 0 , div );
        if( ok( a , div , temp ) && ok( b , div , temp ) ) cnt++;
    }
    cout << cnt << endl;
    return 0;
}
