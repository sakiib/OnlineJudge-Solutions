...
int visited[ 2*N ];
int Oddpath[ 2*N ] ,Evenpath[ 2*N ];
vector < int > ans;
bool cycle;
int vis[ 2*N ] , Even[ 2*N ] , Odd[ 2*N ];

void dfs( int st , int pa , int len ) {
    visited[st]++;
    if( len%2 == 0 ) Even[st] = 1 , Evenpath[st] = pa;
    else Odd[st] = 1 , Oddpath[st] = pa;
    if( graph[st].empty() && len%2 == 1 ) {
        pf("Win\n");
        int k = 0 , win = st;
        while( win != 0 ) {
            ans.pb( win );
            if( k%2 == 0 ) win = Oddpath[win];
            else win = Evenpath[win];
            k++;
        }
        REV( ans );
        FOR( i , 0 , sz( ans )-1 ) pf("%d ",ans[i]);
        exit(0);
    }
    FOR( i , 0 , sz( graph[st] )-1 ) {
        int next = graph[st][i];
        if( visited[next] == 0 )  dfs( next , st , len+1 );
        else if( visited[next] == 1 )  {
            if( Odd[next] && len%2 == 1 ) dfs( next , st , len+1 );
            else if( Even[next] && len%2 == 0 ) dfs( next , st , len+1 );
        }
    }
}
void Cycle( int start ) {
    if( vis[start] == 2 ) return;
    if( vis[start] == 1 ) { cycle = true; return; }
    vis[start] = 1;
