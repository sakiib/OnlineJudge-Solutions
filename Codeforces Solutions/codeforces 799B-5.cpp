...
int main( int argc , char const *argv[] ) {
    ...
    int n; sc("%d",&n);
    FOR( i , 1 , n ) sc("%d",&price[i]);
    FOR( i , 1 , n ) sc("%d",&f[i]);
    FOR( i , 1 , n ) sc("%d",&b[i]);
    FOR( i , 1 , n ) {
        S[ f[i] ].insert( mp(price[i],i) );
        S[ b[i] ].insert( mp(price[i],i) );
    }
    int c; sc("%d",&c);
    set <ii> :: iterator it;
    FOR( i , 1 , c ) {
        ...
        ii P = *S[fav].begin();
        /**cout << "first : " << P.F << " " << P.S << endl;
        for( it = S[fav].begin(); it != S[fav].end(); it++ ) {
            ii x = *it;
            cout <<"In : "<< x.F << " " << x.S << endl;
        }*/
        pf("%d ",P.F);
        int id = P.S;
        // cout << id << " " << f[id] << " " << b[id] << endl;
        S[ f[id] ].erase( P );
        S[ b[id] ].erase( P );
    }
    return 0;
