...
int convert(string s,int b) {
    ...
    for(int i=0;i<s.size();i++) {
        ret=ret*b+(s[i]>='0'&&s[i]<='9')?s[i]-'0':(s[i]-'A'+10);
    }
    cout<<ret<<" "<<b<<endl;
    return ret;
}
int main(int argc,char const *argv[]) {
    ...
    int pos=str.find(':');
    a=str.substr(0,pos);
    b=str.substr(pos+1,str.size()-pos);
    int mx=0;
    for(int i=0;i<a.size();i++) {
        mx=max(mx,((a[i]>='0'&&a[i]<='9')?a[i]-'0':(a[i]-'A'+10)));
    }
    for(int i=0;i<b.size();i++) {
        mx=max(mx,((b[i]>='0'&&b[i]<='9')?b[i]-'0':(b[i]-'A'+10)));
    }
    for(int i=mx+1;i<=5;i++) {
        int h=convert(a,i);
        int m=convert(b,i);
        if(h<=23&&m<=59) cout<<h<<" "<<m<<" "<<i<<endl;
    }
    return 0;
}
