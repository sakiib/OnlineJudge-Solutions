...
bool check_1(int x,int y,int ase,int R,int r) {
    double dist=sqrt(sq(x)+sq(y));
    if(dist>=(ase*1.0)&&dist<=(R*1.0)) return true;
    return false;
}
bool check_2(int x,int y,int ase,int R,int r) {
    double dist=sqrt(sq(x)+sq(y));
    if(dist+r*1.0<=R*1.0) return true;
    return false;
}
bool check_3(int x,int y,int ase,int R,int r) {
    double dist=sqrt(sq(x)+sq(y));
    if(dist-r*1.0>=ase*1.0) return true;
    return false;
}
int main(int argc,char const *argv[]) {
    ...
    for(int i=1;i<=s;i++) {
        ...
        cin>>x>>y>>r;
        int ase=R-d;
        if(check_1(x,y,ase,R,r)&check_2(x,y,ase,R,r)&&check_3(x,y,ase,R,r)) {//baire na & vitore na 1 bad
            ans++;
        }
        else continue;
    }
