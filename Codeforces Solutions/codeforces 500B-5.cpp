...
int main( int argc , char const *argv[] ) {
    ...
    FOR( i , 0 , n-1 ) {
        FOR( j , 0 , n-1 ) {
            if( str[i][j] == '1' ) pos[i+1][j+1] = true;
        }
    }
    FOR( i , 1 , n ) {
        FOR( j , 1 , n ) {
            if( pos[i][j] ) graph[i].pb(j) , graph[j].pb(i);
        }
    }
    FOR( i , 1 , n ) if( !visited[i] ) ++id , dfs( i );
    FOR( i , 1 , id ) {
        vector < int > temp;
        SORT( compo[i] );
        FOR( j , 0 , sz( compo[i] )-1 ) {
            temp.pb( p[compo[i][j]] );
        }
        SORT( temp );
        int idx = 0;
        FOR( k , 0 , sz( compo[i] )-1 ) p[ compo[i][k] ] = temp[idx] , idx++;
    }
    FOR( i , 1 , n ) pf("%d ",p[i]);
    return 0;
}
